...
-- Remove timer cloud entities from realtime publication when present.
do $$
begin
  if exists (
    select 1
    ...
    where pubname = 'supabase_realtime'
      ...
      and tablename = 'timer_laps'
  ) then
    execute 'alter publication supabase_realtime drop table public.timer_laps';
  end if;
  ...
  if exists (
    ...
    where pubname = 'supabase_realtime'
      and schemaname = 'public'
      and tablename = 'timer_sessions'
  ) then
    execute 'alter publication supabase_realtime drop table public.timer_sessions';
  end if;
end;
$$;

-- Drop timer_laps objects before timer_sessions due to FK dependency.
...
drop index if exists public.timer_laps_owner_updated_idx;
drop index if exists public.timer_laps_session_id_idx;
drop table if exists public.timer_laps;
...
drop index if exists public.timer_sessions_owner_updated_idx;
drop index if exists public.timer_sessions_template_id_idx;
drop index if exists public.timer_sessions_plan_day_id_idx;
drop table if exists public.timer_sessions;

-- Recreate pull RPC without timer session/lap entities.
...
-- Recreate tombstone compaction helper without timer session/lap targets.
